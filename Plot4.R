...
## Sets the plotting parameters for the upcoming 4 plots
par(mfrow = c(2,2))
...
## Creates empty plot without x axis thicks or label
with(newdata, plot(1:nrow(newdata), Voltage, type = "n", 
                   ylab = "Voltage", xlab = "datetime", xaxt = "n"))

## Plots the data as a function of time (trick used is to plot against number of 
## rows, as it is simpler than to use the POSIXct date format)
lines(1:nrow(newdata), newdata$Voltage)
...
legend("topright", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
       col = c("black", "red", "blue"), lty = 1, cex = 0.30, bty = "n")

## Bottom right plott

## Creates empty plot without x axis thicks or label
with(newdata, plot(1:nrow(newdata), Global_reactive_power, type = "n", 
                   ylab = "Global_reactive_power", xlab = "datetime", xaxt = "n"))

## Plots the data as a function of time (trick used is to plot against number of 
## rows, as it is simpler than to use the POSIXct date format)
lines(1:nrow(newdata), newdata$Global_reactive_power)
